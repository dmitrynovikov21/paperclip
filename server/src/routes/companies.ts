...
export function companyRoutes(db: Db, storage?: StorageService, options?: CompanyRoutesOptions) {
  ...
  router.put(
    ...
    async (req, res) => {
      ...
      await companyTransferRunService.completePart(db, run.id, partName);
  ...
  router.post("/import/transfers/:transferId/apply", async (req, res) => {
    ...
    await executeImportRequest(req, res, rawImportBody, {
      ...
      onFailure: async (message) => {
        // Parts remain spooled: the failure is in the import itself, not the
        // upload, so a retry of apply can reuse the verified parts.
        await companyTransferRunService.fail(db, run.id, message);
      },
    });
  });

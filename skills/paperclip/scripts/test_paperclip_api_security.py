...
class ApiHelperSecurityTest(unittest.TestCase):
    ...
    def test_artifact_uploader_reads_file_with_quote_and_backslash(self):
        ...
        upload.write_text("safe-upload-payload")
        env = dict(self.env, PAPERCLIP_HELPER_STATE_DIR=self.scratch.name + "/locks")
        result = subprocess.run(
            ["bash", str(UPLOAD_HELPER), str(upload), "--content-type", "text/plain",
             "--no-work-product", "--output", "json", "--company-id", "c", "--issue-id", "x"],
            env=env,
            capture_output=True,
            text=True,
            timeout=15,
        ...
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(len(self.victim.requests), 2)
        self.assertEqual(len(self.attacker.requests), 0)
